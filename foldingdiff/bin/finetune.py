# finetune model, using a pretrained model
# take in arguments for type of training method, e.g. PPO, TRPO, etc 
"""
Training script.
...
def train_again(
    from_ckpt_dir: str = "", # aka old_results_dir
    new_results_dir: str = "",
    # TODO: From here, ensure all the parameters are the same as the original training script! 
    ...
    np.save(
        new_results_folder / "training_mean_offset.npy",
    ...
    callbacks = build_callbacks(
        outdir=new_results_folder, early_stop_patience=early_stop_patience, swa=use_swa
    )
    ...
    trainer = pl.Trainer(
        default_root_dir=new_results_folder,
...
def train_old(
    from_dir: str = "wukevin/foldingdiff_cath",
    # Controls output
    ...
    time_encoding: finetuning.TIME_ENCODING = "gaussian_fourier",
    ...
    cfg = BertConfig( # TODO: old config, need to modify to finetune
    ...
    if from_dir:
        model = finetuning.BertForDiffusion.from_dir(snapshot_download("wukevin/foldingdiff_cath"))
    else:
        model = finetuning.BertForDiffusion(
            config=cfg,
    ...
    trainer.fit(
        model=model,
        train_dataloaders=train_dataloader, # these have samples 
